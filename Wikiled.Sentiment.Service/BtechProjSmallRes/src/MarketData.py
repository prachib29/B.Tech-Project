...
class MarketData(object):
    ...
    def get_binary_data(self, df, days=5, from_date='2014-04-01', to_date='2015-04-01'):
        ...
        df = df.loc[from_date:to_date].copy()
        df.to_csv("1.csv")
        self.df.loc[from_date:to_date].to_csv("2.csv")
        
        df_full = pd.merge(df, self.df.loc[from_date:to_date], left_index=True, right_index=True, how='outer')
    ...
    def get_stock_data(self):
        df = self.df.copy()
        # Percentage change(pct_change() function Computes the percentage change from the immediately previous row by default.
...
class QuandlMarketDataSource(object):
    ...
    def get_stock_data(self, stock_name):
        """
                ...
                """
        quandl.ApiConfig.api_key = '5gFC-zqWmuT4u4nQSmqU'
        #print("Processing")
        '''ticker name is used to identify comapny name in stocks, like 'JPM' is a tikcer'''
